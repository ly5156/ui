...
export default Component.extend(StickyHeader, {
  ...
  selectValueChanged: observer('selectValue', function() {
    this.sendAction('selectChange', get(this, 'selectValue'));
  }),
  ...
  actionsChanged: observer('selectedNodes.[]', function() {
    ...
    if (disableAll) {
      ...
      get(this, 'availableActions').forEach((a) => {
        if (a.bulkable === false) {
          set(a, 'enabled', nodes.length === 1);
        } else {
          set(a, 'enabled', a.active ? a.active.call(this, nodes) : true);
        }
      });
    }
    this.sendAction('selectionChanged', nodes);
  }),
  ...
  hasNextData: computed('nextData', function() {
    return !!get(this, 'nextData');
  }),
