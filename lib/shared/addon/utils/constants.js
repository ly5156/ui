...
C.COMMON_RULES = [
  ...
  },
  {
    ...
    value:                  'pandaria_etcd_is_backup_failed',
    scope:                  'cluster',
    comparison:             C.ALERTING_COMPARISON.GREATER_THAN,
    thresholdValue:         0,
    onlyLocal:              true,
    onlyReadThresholdValue: true
  },
  {
    label:          'cpuUsagePercent',
    value:          '(sum(container_cpu_usage_seconds_total) by (pod_name, container_name) / sum(label_join(label_join(kube_pod_container_resource_limits_cpu_cores,"pod_name", "", "pod"),"container_name", "", "container")) by (pod_name, container_name)) * 100',
    scope:          'project',
    comparison:     C.ALERTING_COMPARISON.GREATER_OR_EQUAL,
    thresholdValue: 100,
    unit:           'percent'
  },
  {
    ...
    value:          '(sum(container_memory_working_set_bytes) by (pod_name, container_name) / sum(label_join(label_join(kube_pod_container_resource_limits_memory_bytes,"pod_name", "", "pod"),"container_name", "", "container")) by (pod_name, container_name)) * 100',
    scope:          'project',
    comparison:     C.ALERTING_COMPARISON.GREATER_OR_EQUAL,
    thresholdValue: 100,
    unit:           'percent'
