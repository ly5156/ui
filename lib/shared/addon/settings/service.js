...
export default Service.extend(Evented, {
  ...
  setUnknownProperty(key, value) {
    if (key !== 'app') {
      ...
      if (key === 'auditlog-server-url') {
        if (value === 'forums.rancher.com') {
          return
        }
        let urlReg = /[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+\.?/;
        let subUrl = urlReg.exec( value )

        get(this, 'globalStore').rawRequest({
          url:     `/v3/settings/whitelist-domain`,
          method:  'GET',
        }).then( (data) => {
          if (data.status === 200) {
            let whitelistValue = ''
            ...
            if (!value) {
              let arr = data.body.value.split(',');
              let whitelistArr = arr.filter((item) => !(obj.get('value').indexOf(item) > -1) );

              whitelistValue = whitelistArr.join(',')
            } else if (subUrl) {
              let arr = data.body.value.split(',');
              let hasBeenWhiteValue = arr.some((item) => subUrl[0].indexOf(item) > -1 );

              hasBeenWhiteValue ? whitelistValue = data.body.value : whitelistValue = `${ data.body.value },${ subUrl[0] }`
            } else {
              whitelistValue = `${ data.body.value },${ value }`
            }
            let params = Object.assign(data.body, { value: whitelistValue })
            ...
            get(this, 'globalStore').rawRequest({
              ...
              if (data.body && data.body.message) {
                get(this, 'growl').fromError(data.body.message);
              } else {
                get(this, 'growl').fromError('未知错误');
              }
            })
          }
        }).catch( (data) => {
  ...
  globalMonitoringClusterId:  alias(`asMap.${ C.SETTING.GLOBAL_MONITORING_CLUSTER_ID }.value`),
  globalMonitoringEnabled:    alias(`asMap.${ C.SETTING.GLOBAL_MONITORING_ENABLED }.value`),
  ...
  extraMenus:                 alias(`asMap.${ C.SETTING.EXTRA_MENUS }.value`),
  ...
  footerText: computed(C.SETTING.FOOTER_TEXT, function() {
    const out = get(this, C.SETTING.FOOTER_TEXT) || ''

    return out;
  }),

  footerUrl: computed(C.SETTING.FOOTER_TEXT, function() {
    let out = get(this, C.SETTING.FOOTER_URL) || ''

    if ( out && !(out.startsWith('https://') || out.startsWith('http://')) ) {
      out = `https://${ out }`
    }
