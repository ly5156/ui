...
export default Component.extend(ThrottledResize, {
  ...
  create() {
    ...
    const chart = ECharts.init(element, 'walden');

    set(this, 'chart', chart);
    set(this, 'chartEle', element);
    chart.showLoading(LOADING_PARAMS);
    this.draw();
    element.onmousemove = (ev) => {
      set(this, 'mouseOffset', [ev.pageX, ev.pageY]);
    }
  },
    ...
    let option = {
      tooltip: {
        ...
        position(pos, params, dom, rect, size) {
          const mouseOffset = that.mouseOffset;
          const d = that.$(dom);
          const body = d.closest('body')[0];
          let  obj = {};
          let distance = body.offsetHeight - mouseOffset[1] - footer + pos[1]

          obj[['left', 'right'][+(pos[0] < size.viewSize[0] / 2)]] = 5;
          if (body.offsetHeight - 2 * footer > size.contentSize[1] ){
            obj.top = body.offsetHeight - mouseOffset[1] - footer < size.contentSize[1] ? distance - size.contentSize[1] : pos[1];
          } else {
            let maxHeight = body.offsetHeight - 2 * footer;

            obj.top = distance - maxHeight;
            d.css({
              overflow: 'auto',
              height:   `${ maxHeight }px`,
            })
        ...
        formatter(params) {
          ...
          params.sort((a, b) => b.data[1] - a.data[1]).forEach((p, i) => {
            if ( i === 0 ) {
  ...
  willDestroy() {
    this._super(...arguments);
    this.chartEle.onmousemove = null
    set(this, 'chartEle', null)
  },
});
