...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  gpuSchedulerNodePortChanged: observer('rkeSchedulerConfig', 'cluster.gpuSchedulerNodePort', 'cluster.enableGPUManagement', function() {
    // set gpu sharing
  ...
  schedulerPolicyConfigStr: computed('cluster.gpuSchedulerNodePort', 'schedulerPolicyConfig', function() {
    const port = get(this, 'cluster.gpuSchedulerNodePort');
    ...
    if (clusterTemplateQuestions && clusterTemplateQuestions.length > 0) {
      clusterTemplateQuestions.forEach((question) => {
        ...
        if (!question.variable.includes('uiOverride') && question.default) {
          ...
          if (path === 'rancherKubernetesEngineConfig.network.plugin' && defaultValue === 'none') {
            defaultValue = get(primaryResource, 'rancherKubernetesEngineConfig.network.options.pandariaExtraPluginName');
          }
