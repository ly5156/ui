...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  init() {
    ...
    this.initNodeCounts();
    this.loadSchedulerPolicyConfig();
    this.loadRkeSchedulerConfig();
  ...
  gpuNodePortChanged: observer('rkeSchedulerConfig', 'gpuNodePort', 'cluster.enableGPUManagement', function() {
    // set gpu sharing
    if (get(this, 'cluster.enableGPUManagement')) {
      const annotations = get(this, 'cluster.annotations') || {};

      annotations['gpuschedulerport.cattle.io'] = `${ get(this, 'gpuNodePort') }`;
      set(this, 'cluster.annotations', annotations);
      const services = get(this, 'cluster.rancherKubernetesEngineConfig.services') || {};
      const scheduler = services.scheduler || {};

      scheduler.extraArgs = scheduler.extraArgs || {};
      scheduler.extraBinds = scheduler.extraBinds || [];

      const schedulerConfig = get(this, 'rkeSchedulerConfig');

      if (schedulerConfig) {
        Object.assign(scheduler.extraArgs, schedulerConfig.scheduler.extraArgs);
        scheduler.extraBinds.push(...schedulerConfig.scheduler.extraBinds);
        set(services, 'scheduler', scheduler);
      }
    } else {
      const services = get(this, 'cluster.rancherKubernetesEngineConfig.services') || {};
      const scheduler = services.scheduler || {};

      if (scheduler.extraArgs && scheduler.extraArgs['policy-config-file']) {
        delete scheduler.extraArgs['policy-config-file']
      }
      if (scheduler.extraBinds && scheduler.extraBinds.length > 0) {
        scheduler.extraBinds = scheduler.extraBinds.filter((item) => item !== '/etc/gpushare/scheduler-policy-config.json:/etc/gpushare/scheduler-policy-config.json');
      }
    }
  }),
  ...
  schedulerPolicyConfigStr: computed('gpuNodePort', 'schedulerPolicyConfig', function() {
    const port = get(this, 'gpuNodePort');

    return get(this, 'schedulerPolicyConfig').replace(':_NODEPORT_', `${ port ? `:${ port }` : '' }`);
  }),

  choseMultusFlannel: computed('cluster.rancherKubernetesEngineConfig.network.plugin', function() {
    return get(this, 'cluster.rancherKubernetesEngineConfig.network.plugin') === MULTUS_FLANNEL;
  }),

  choseMultusCanal: computed('cluster.rancherKubernetesEngineConfig.network.plugin', function() {
    return get(this, 'cluster.rancherKubernetesEngineConfig.network.plugin') === MULTUS_CANAL;
  }),
  ...
  willSave() {
    ...
    if (ok) {
      ...
      // multus_flanel and mutus_canal config
      const options = get(this, 'cluster.rancherKubernetesEngineConfig.network.options');
      const plugin = get(this, 'cluster.rancherKubernetesEngineConfig.network.plugin');

      if ( options && options.flannel_iface !== undefined && plugin !==  MULTUS_FLANNEL) {
        delete options.flannel_iface
      }
      if ( options && options.canal_iface !== undefined && plugin !==  MULTUS_CANAL) {
        delete options.canal_iface
      }
    }
  ...
  initGpuConfigs() {
    const enableGPUManagement = get(this, 'cluster.enableGPUManagement');
    const annotations = get(this, 'cluster.annotations') || {};

    if (enableGPUManagement) {
      set(this, 'gpuNodePort', annotations['gpuschedulerport.cattle.io'] || 32666);
    } else {
      delete annotations['gpuschedulerport.cattle.io'];
    }
  },
    ...
    if (clusterTemplateQuestions && clusterTemplateQuestions.length > 0) {
      clusterTemplateQuestions.forEach((question) => {
        ...
        if (!question.variable.includes('uiOverride') && question.default) {
          ...
          case 'boolean':
            defaultValue = typeof defaultValue === 'string' ? defaultValue.toLowerCase() === 'true' : defaultValue;
            break;
          case 'int':
            defaultValue = parseInt(defaultValue, 10);
            break;
          }
          deepSet(primaryResource, path, defaultValue);
        }
  ...
  loadSchedulerPolicyConfig() {
    return get(this, 'globalStore').rawRequest({
      url:     '/gpusharing/templates/schedulerpolicyconfig',
      method:  'GET',
    }).then((resp) => {
      set(this, 'schedulerPolicyConfig', resp.body);
    });
  },
  loadRkeSchedulerConfig() {
    return get(this, 'globalStore').rawRequest({
      url:     '/gpusharing/templates/rkeschedulerconfig',
      method:  'GET',
    }).then((resp) => {
      set(this, 'rkeSchedulerConfig', resp.body);
    });
  },
});
