import { resolve, reject, all, Promise } from 'rsvp';
...
export default Service.extend({
  ...
  detect() {
    ...
    return hash({
      ...
      footerText:   globalStore.request({ url: `settings/${ C.SETTING.FOOTER_TEXT }` }),
      footerURL:    globalStore.request({ url: `settings/${ C.SETTING.FOOTER_URL }` }),
      ...
      providers, pl, footerText, footerURL, firstSetting
    }) => {
      ...
      if ( footerText ) {
        this.settings.notifyPropertyChange(C.SETTING.FOOTER_TEXT);
      }

      if ( footerURL ) {
        this.settings.notifyPropertyChange(C.SETTING.FOOTER_URL);
      }
  ...
  clearToken() {
    const p = (get(this, 'providers') || []).findBy('id', get(this, 'principal.provider'));

    if (p && p.logoutUrl) {
      return all([this.thirdAuthLogout(p.logoutUrl),
  ...
  thirdAuthLogout(url) {
    return new Promise(((resolve, reject) => {
      if (!url.startsWith(window.location.protocol)) {
        window.open(url, 'rancherAuthLogout', Util.popupWindowOptions());
